      ...
      <div class="checkout-tit">
        ...
        <div class="paymark">
          ...
          <span class="fr"
            ...
            ><em class="orange money">{{ ProInformation.totalFee }}</em></span
          >
        </div>
      </div>
...
export default {
  ...
  methods: {
    async getcreateNative() {
      let result = await this.$API.reqgetcreateNative(this.$route.query);
      if (result.code == 200) {
        this.ProInformation = result.data;
        return "ok";
      } else {
    ...
    async open() {
      let url = await QRCode.toDataURL(this.ProInformation.codeUrl);
      ...
      if (!this.timer) {
        this.timer = setInterval(async () => {
          //发请求获取用户支付状态
          let result = await this.$API.reqgetQueryPayStatus(this.ProInformation.orderId);
          console.log(result)
          //如果code==200
          if (result.code == 200) {
            //第一步：清除定时器
