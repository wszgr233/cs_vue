          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spuSale, index) in spuSaleAttrList" :key="index">
                ...
                <dd
                  changepirce="0"
                  :class="{ active: ValueList.id == 9 }"
                  v-for="(ValueList, index) in spuSale.spuSaleAttrValueList"
                  :key="ValueList.id"
                >
            ...
            <div class="cartWrap">
              <div class="controls">
                <input
                  autocomplete="off"
                  class="itxt"
                  v-model="skuNameIndex"
                  @change="changeskuName"
                />
                <a href="javascript:" class="plus" @click="skuNameIndex++">+</a>
                <a
                  href="javascript:"
                  class="mins"
                  @click="
                    skuNameIndex > 1 ? skuNameIndex-- : (skuNameIndex = 1)
                  "
                  >-</a
                >
              </div>
...
export default {
  ...
  mounted() {
    this.$store.dispatch("getItemList", this.$route.params.suikId);
  },
  computed: {
  ...
  methods: {
    changeskuName() {
      let val = event.target._value * 1;
      if (isNaN(val) || val < 1) {
        this.skuNameIndex = 1;
      } else {
        this.skuNameIndex = parseInt(val);
      }
    },
    async addToCart() {
      let location ={name: "addcartsuccess"}
      let query={skuNameIndex:this.skuNameIndex}
      sessionStorage.setItem("skuInfo",JSON.stringify(this.skuInfo))
      location.query=query
      try {
       await this.$store.dispatch("addToCart", {
          skuId: this.$route.params.suikId,
          skuName: this.skuNameIndex,
        });
